...
const CandidatePostFormExtra = props => {
    ...
    const getcategoriesitemvalues = async () => {
    ...
    const [HaveImageAdhar, setHaveImageAdhar] = useState(false);
    const [HaveImageAdhar2, setHaveImageAdhar2] = useState(true);

    const [PoliciesfilesErrorMessageadhar, setPoliciesfilesErrorMessageadhar] = useState();
    const [imageNameadhar, setImageNameadhar] = useState('');
    ...
    const onEmpadharphoto = (data) => {
        const updatedImageValueadhar = data.target.files[0].name
        const name = data.target.files[0].name;
        const lastDot = name.lastIndexOf('.');
        const ext = name.substring(lastDot + 1);
        setHaveImageAdhar2(false)
        if (ext === "jpg" || ext === "jpeg" || ext === "png" || ext === "pdf") {

        } else {
            setValue("adharphoto", null)
            setHaveImageAdhar(false)
            setPoliciesfilesErrorMessageadhar('Please select valid file')
            return false
        }
        ...
        if (data.target.files[0].size > 2e6) {
            setValue("adharphoto", null)
            setHaveImageAdhar(false)
            setPoliciesfilesErrorMessageadhar('Please select lower 2mb file')
            return false;
        }

        if (data.target.files.length === 1) {
            setPoliciesfilesErrorMessageadhar('')
            setHaveImageAdhar(true)
            setImageNameadhar(updatedImageValueadhar)
        } else {
            setPoliciesfilesErrorMessageadhar('This field is required')
            setHaveImageAdhar(false)
            setImageNameadhar("")
            setImageNameadhar('')
        }
    }
    const removeFileFromHereadhar = () => {
        setValue("adharphoto", "");
        setHaveImageAdhar(false);
    }
    ...
    const onSubmit = async (data) => {
        ...
        try {
            const response = await axios.post(`${process.env.REACT_APP_CONDIDATES}/api/v1/candidates/`, formData, {
                      ...
                      /> */}
                                        ...
                                        <Row className="mt-2 align-items-center">
                                            ...
                                            <Col xl={9}>
                                                ...
                                                />
                                                {/* {errors.adharphoto && (<small id='profilePhotoHelp' className='form-text text-danger'>    {errors.adharphoto.message} </small>)} */}
                                                <small className="text-danger"> {PoliciesfilesErrorMessageadhar}</small>
                                                <small className="text-muted">Eg:  (jpeg,png,pdf,jpg)</small>
                                                {HaveImageAdhar ? (
                                                    <div className=''>
                                                        <p className="mt-1 font_13">{imageNameadhar}  <span
                                                            className='text-danger'
                                                            onClick={() => removeFileFromHereadhar()}>
                                                            <i className='fa fa-close'></i>
                                                        </span> </p>
...
export default CandidatePostFormExtra;
